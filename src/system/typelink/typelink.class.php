...
class TypeLink
{
    ...
    function GetOptionList($hid = 0, $oper = 0, $channeltype = 0, $usersg = 0)
    {
        ...
        if ($hid > 0) {
            ...
            if ($row['ispart'] == 1) {
                $this->OptionArrayList .= "<option value='".$row['id']."' style='background:#f8f8f8;color:#545b62' selected>└─ ".$row['typename']."</option>\r\n";
            } else {
        ...
        while ($row = $this->dsql->GetObject()) {
            if ($row->id != $hid) {
                if ($row->ispart == 1) {
                    $this->OptionArrayList .= "<option value='".$row->id."' style='background:#f8f8f8;color:#545b62'>└─ ".$row->typename."</option>\r\n";
                } else {
    ...
    function LogicGetOptionArray($id, $step, $oper = 0)
    {
        ...
        while ($row = $this->dsql->GetObject($id)) {
            ...
            if ($row->ispart == 1) {
                $this->OptionArrayList .= "<option value='".$row->id."' style='background:#f8f8f8;color:#545b62'>└─$step ".$row->typename."</option>\r\n";
            } else {
