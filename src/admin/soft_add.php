...
if ($dopost != 'save') {
    ...
    if ($softurl1 != '') {
        ...
        if ($autosize && empty($softsize)) {
            ...
            if (empty($nsoftsize)) $nsoftsize = Lang('unknow');
            else {
    ...
    else if (empty($softsize)) $softsize = Lang('unknow');
    ...
    for ($i = 2; $i <= 30; $i++) {
        if (!empty(${'softurl'.$i})) {
            ...
            if ($servermsg == '') $servermsg = Lang('download_url').$i;
            if ($softurl != 'http://') {
